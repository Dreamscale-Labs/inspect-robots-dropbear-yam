...
def _doctor_command(json_output: bool, support_bundle: Path | None) -> int:
    rig = load_rig()
    report = doctor(rig)
    if json_output:
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        if args.command == "setup":
            setup(reconfigure=args.reconfigure)
            return 0
        if args.command == "doctor":
            return _doctor_command(args.json_output, args.support_bundle)
        if args.command == "run":
            rig = load_rig()
            return run(
                args.instruction,
